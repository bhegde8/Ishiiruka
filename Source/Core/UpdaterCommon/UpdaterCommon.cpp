...
struct TodoList
{
  ...
  {
    Manifest::Filename filename;
    nonstd::optional<Manifest::Hash> old_hash;
...
bool HexDecode(const std::string& hex, u8* buffer, size_t size)
{
  ...
  auto DecodeNibble = [](char c) -> nonstd::optional<u8> {
    if (c >= '0' && c <= '9')
  ...
  for (size_t i = 0; i < size; ++i)
  {
    nonstd::optional<u8> high = DecodeNibble(hex[2 * i]);
    nonstd::optional<u8> low = DecodeNibble(hex[2 * i + 1]);
...
nonstd::optional<std::string> GzipInflate(const std::string& data)
{
  z_stream zstrm;
...
Manifest::Hash ComputeHash(const std::string& contents)
{
  ...
  mbedtls_sha256(reinterpret_cast<const u8*>(contents.data()), contents.size(), full.data(),
                     false);
                     ...
                     const std::string& content_base_url, const std::string& temp_path)
{
  ...
  for (size_t i = 0; i < to_download.size(); i++)
  {
    ...
    nonstd::optional<std::string> maybe_decompressed = GzipInflate(contents);
...
nonstd::optional<std::string> FindOrCreateTempDir(const std::string& base_path)
{
                 ...
                 const std::string& install_base_path, const std::string& temp_path)
{
  for (const auto& op : to_update)
  {
    ...
    nonstd::optional<mode_t> permission;
#endif
...
nonstd::optional<Manifest> ParseManifest(const std::string& manifest)
{
  Manifest parsed;
  size_t pos = 0;
...
nonstd::optional<Manifest> FetchAndParseManifest(const std::string& url)
{
...
struct Options
{
  ...
  nonstd::optional<std::string> binary_to_restart;
  nonstd::optional<u32> parent_pid;
  nonstd::optional<std::string> log_file;
};

nonstd::optional<Options> ParseCommandLine(std::vector<std::string>& args)
{
...
{
  nonstd::optional<Options> maybe_opts = ParseCommandLine(args);
  ...
  {
    nonstd::optional<Manifest> maybe_manifest = FetchAndParseManifest(opts.this_manifest_url);
    if (!maybe_manifest)
    {
  ...
  nonstd::optional<std::string> maybe_temp_dir = FindOrCreateTempDir(opts.install_base_path);
  if (!maybe_temp_dir)
    return false;
